...
export type GameEvent = {
  type: string; // "START" | "HIT" | "MISS" | "END"
  ts: number;
...
export type GameProps = {
  ...
  onScore?: (s: GameScore) => void;
};

// ==== Bridge pro iSenses ===================================
function useISensesBridge(props: GameProps) {
  const { onEvent, onScore } = props;
  const [sessionId, setSessionId] = useState(props.sessionId || "");
  const [config, setConfig] = useState<Record<string, any>>(props.config || {});
...
// ==== Herní logika ==========================================
const SEQUENCE: string[] = ["A", "P", "B", "R", "K", "H", "L", "M", "N", "R"];
...
// ==== Komponenta ============================================
export default function Task1AccommodationGame(props: GameProps) {
  ...
  const layout = (config?.layout as "row" | "grid") || "row";

  // Písmena se na obrazovce zobrazí v náhodném pořadí (pool)
  const [pool, setPool] = useState<string[]>(() => shuffle(SEQUENCE));
  // Index v SEQUENCE – určuje, které písmeno má klient teď najít (pořadí z desky)
  const [targetIndex, setTargetIndex] = useState<number>(0);
  // Označení dokončených cílů podle SEQUENCE (10 položek)
  const [done, setDone] = useState<boolean[]>(() => new Array(SEQUENCE.length).fill(false));
  const [running, setRunning] = useState(false);
  const [errors, setErrors] = useState(0);

  const startTs = useRef<number | null>(null);
  const lastTargetStart = useRef<number | null>(null);
  const reactionList = useRef<number[]>([]);
  const touchLog = useRef<
    Array<{
      letter: string;
      targetXY: { x: number; y: number };
      touchXY: { x: number; y: number };
      distance: number;
    }>
  >([]);
  ...
  const start = useCallback(() => {
    ...
    emitEvent({
      type: "START",
      ts: now,
      data: { sessionId, taskId, sequence: SEQUENCE.join("") },
    });
  ...
  const finish = useCallback(() => {
    ...
    emitScore({
      ...
      details: {
        ...
        touchLog: touchLog.current,
      },
    });
  }, [errors, taskId, emitEvent, emitScore]);

  const onTileClick = useCallback(
    (letter: string, ev: React.MouseEvent<HTMLButtonElement>) => {
      if (!running) return;
      ...
      const targetXY = rectStage
        ? {
            x: rectBtn.left - rectStage.left + rectBtn.width / 2,
            y: rectBtn.top - rectStage.top + rectBtn.height / 2,
          }
        : { x: 0, y: 0 };
      ...
      if (isCorrect) {
        ...
        emitEvent({
          type: "HIT",
          ts: Date.now(),
          data: { letter, reactionMs: rt, targetXY, touchXY, distancePx: dist },
        });
        ...
        emitEvent({
          type: "MISS",
          ts: Date.now(),
          data: { expected: currentLetter, clicked: letter, targetXY, touchXY, distancePx: dist },
        });
        touchLog.current.push({ letter, targetXY, touchXY, distance: dist });
      }
    },
  ...
  // UI dlaždic – náhodné pořadí, vizualizace „hotovo“ i s duplicitou 'R'
  const Tiles = (
    <div
      style={{
        display: "flex",
    ...
    >
      {pool.map((letter, idx) => {
        // Kolik výskytů tohoto písmene už patří do minulosti dle SEQUENCE?
  ...
  return (
    ...
    >
      ...
      {/* Herní plocha se středovým červeným čtvercem */}
