...
export default function AccommodationGame(props) {
  ...
  const finish = useCallback(() => {
    ...
    const accuracy =
      totalClicks > 0 ? Math.round((correct / totalClicks) * 100) : 100;
  ...
  const onTileClick = useCallback(
    (letter, ev) => {
      ...
      if (isCorrect) {
        ...
        const rt = lastTargetStart.current
          ? Math.round(nowp - lastTargetStart.current)
          : 0;
  ...
  const Tiles = (
    ...
    >
      {pool.map((letter, idx) => {
        const expectedCountBefore = SEQUENCE.slice(0, targetIndex + 1).filter(
          (l) => l === letter
        ).length;
        const actualDoneCount = SEQUENCE.map((l, i) => ({ l, i })).filter(
          (x) => x.l === letter && done[x.i]
        ).length;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          krok: {Math.min(targetIndex + 1, SEQUENCE.length)}/{SEQUENCE.length}
        </div>
      </div>
